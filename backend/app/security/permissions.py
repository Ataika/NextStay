...
def assert_can_invite_role(actor_role: str | None, target_role: str) -> None:
    ...
    if target not in ASSIGNABLE_USER_ROLES:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid role. Must be one of: {sorted(ASSIGNABLE_USER_ROLES)}")
...
def assert_can_change_role(actor_role: str | None, target_current_role: str, target_new_role: str) -> None:
    ...
    if new_role == "MANAGER" and not can_invite_manager(actor_role):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the hotel owner can assign the manager role.")
...
def assert_can_delete_user(actor_role: str | None, target_role: str, *, actor_id: int, target_id: int) -> None:
    if not can_delete_user_account(actor_role):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only the hotel owner can delete user accounts.")
    if actor_id == target_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You cannot delete your own account.")
    if normalize_role(target_role) in PROTECTED_USER_ROLES:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Owner and system accounts cannot be deleted.")
