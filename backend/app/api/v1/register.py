...
def register_owner(payload: RegisterOwnerRequest, db: Session = Depends(get_db)):
    ...
    # Canonical hotels row (identity + sync), then its 1:1 hotel_profile (shared PK).
    hotel_entity = HotelModel(
        code=f"H-{secrets.token_hex(6)}",
        name=hotel_name,
        active=True,
    )
    db.add(hotel_entity)
    db.flush()
    hotel = HotelProfileModel(id=hotel_entity.id, hotel_name=hotel_name)
    db.add(hotel)
    db.flush()
