...
CREATE TABLE IF NOT EXISTS hotel_sync_events (
    ...
    CONSTRAINT hotel_sync_events_status_chk CHECK (status IN ('received', 'processed', 'failed')),
...
CREATE INDEX IF NOT EXISTS idx_hotel_sync_events_hotel_received
    ON hotel_sync_events(hotel_id, received_at DESC);

CREATE INDEX IF NOT EXISTS idx_hotel_sync_events_booking
    ON hotel_sync_events(booking_id);
...
CREATE TABLE IF NOT EXISTS hotel_channel_bookings (
    ...
    CONSTRAINT hotel_channel_bookings_status_chk CHECK (status IN ('active', 'cancelled')),
    CONSTRAINT hotel_channel_bookings_unique_external UNIQUE (hotel_id, external_booking_id)
);

CREATE INDEX IF NOT EXISTS idx_hotel_channel_bookings_booking
    ON hotel_channel_bookings(booking_id);
